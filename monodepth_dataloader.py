...
def rescale_intrinsics(raw_cam_mat, opt, orig_height, orig_width):
  ...
  cx = raw_cam_mat[0,2]
  cy = raw_cam_mat[1,2]
  r1 = tf.stack([fx * opt.img_width/orig_width, 0, cx * opt.img_width/orig_width])
  r2 = tf.stack([0, fy * opt.img_height/orig_height, cy * opt.img_height/orig_height])
...
class MonodepthDataloader(object):
    ...
    def __init__(self, opt):
        ...
        left_image_o, orig_height, orig_width  = self.read_image(left_image_path, get_shape=True)
        ...
        next_left_image  = tf.cond(do_flip > 0.5, lambda: tf.image.flip_left_right(next_right_image_o), lambda: next_left_image_o)
        next_right_image = tf.cond(do_flip > 0.5, lambda: tf.image.flip_left_right(next_left_image_o),  lambda: next_right_image_o)
...
#                                                                              lambda: image_list)
        ...
        last_line = tf.string_split([raw_cam_contents], delimiter="\n").values[-1]
        raw_cam_vec = tf.string_to_number(tf.string_split([last_line]).values[1:])
...
#         raw_cam_mat = raw_cam_mat_batch[0,:,:]
        ...
        self.data_batch = tf.train.shuffle_batch([left_image, right_image, 
                                                  next_left_image, next_right_image,
