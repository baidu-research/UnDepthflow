...
class MonodepthModel(object):
    ...
    def __init__(self, params, mode, left, right, left_feature, right_feature, 
...
#         self.build_summaries()     
    ...
    def generate_transformed(self, img, flow, scale):
        return transformer_old(img, flow, out_size=[self.params.height // (2**scale), 
    ...
    def get_disparity_smoothness_2nd(self, disp, pyramid):
        ...
        weights_x = [tf.exp(-tf.reduce_mean(10.0*tf.abs(g), 3, keep_dims=True)) for g in image_gradients_x]
        weights_y = [tf.exp(-tf.reduce_mean(10.0*tf.abs(g), 3, keep_dims=True)) for g in image_gradients_y]

        smoothness_x = [disp_gradients_xx[i] * weights_x[i][:,:,:-1,:] for i in range(4)]
        smoothness_y = [disp_gradients_yy[i] * weights_y[i][:,:-1,:,:] for i in range(4)]
        return smoothness_x + smoothness_y
    ...
    def build_pwc(self):
        self.disp1, self.disp2, self.disp3, self.disp4 = pwc_disp(self.left, self.right, 
                                                                  self.left_feature, self.right_feature)
    ...
    def build_outputs(self):
        ...
        self.ltr_flow = [self.generate_flow_left(self.disp_left_est[i], i) for i in range(4)]
        self.rtl_flow = [self.generate_flow_right(self.disp_right_est[i], i) for i in range(4)]
        
        self.right_occ_mask = [tf.clip_by_value(transformerFwd(tf.ones(shape=[self.params.batch_size, 
                                                                              H//(2**i), 
                                                                              W//(2**i), 1], dtype='float32'), 
                                                  self.ltr_flow[i], [H//(2**i), W//(2**i)]),
                                   clip_value_min=0.0, clip_value_max=1.0) for i in range(4)]
        
        self.left_occ_mask = [tf.clip_by_value(transformerFwd(tf.ones(shape=[self.params.batch_size, 
                                                                              H//(2**i), 
                                                                              W//(2**i), 1], dtype='float32'), 
                                                  self.rtl_flow[i], [H//(2**i), W//(2**i)]),
                                   clip_value_min=0.0, clip_value_max=1.0) for i in range(4)]
        
        self.right_occ_mask_avg = [tf.reduce_mean(self.right_occ_mask[i]) + 1e-12 for i in range(4)]
        self.left_occ_mask_avg = [tf.reduce_mean(self.left_occ_mask[i]) + 1e-12 for i in range(4)]
        
        # GENERATE IMAGES
        with tf.variable_scope('images'):
            self.left_est  = [self.generate_transformed(self.right_pyramid[i], self.ltr_flow[i], i)  for i in range(4)]
            self.right_est = [self.generate_transformed(self.left_pyramid[i], self.rtl_flow[i], i) for i in range(4)]

        # LR CONSISTENCY
        with tf.variable_scope('left-right'):
            self.right_to_left_disp = [self.generate_transformed(self.disp_right_est[i], self.ltr_flow[i], i)  for i in range(4)]
            self.left_to_right_disp = [self.generate_transformed(self.disp_left_est[i], self.rtl_flow[i], i) for i in range(4)]

        # DISPARITY SMOOTHNESS
        with tf.variable_scope('smoothness'):
            self.disp_left_smoothness  = self.get_disparity_smoothness_2nd(self.disp_left_est,  self.left_pyramid)
            self.disp_right_smoothness = self.get_disparity_smoothness_2nd(self.disp_right_est, self.right_pyramid)
    ...
    def build_losses(self):
        with tf.variable_scope('losses', reuse=self.reuse_variables):
            ...
            # L1
            self.l1_left = [tf.abs(self.left_est[i] - self.left_pyramid[i])*self.left_occ_mask[i] for i in range(4)]
            self.l1_reconstruction_loss_left  = [tf.reduce_mean(l) / self.left_occ_mask_avg[i] for i, l in enumerate(self.l1_left)]
            
            self.l1_right = [tf.abs(self.right_est[i] - self.right_pyramid[i])*self.right_occ_mask[i] for i in range(4)]
            self.l1_reconstruction_loss_right = [tf.reduce_mean(l) / self.right_occ_mask_avg[i] for i, l in enumerate(self.l1_right)]

            # SSIM
            self.ssim_left = [self.SSIM(self.left_est[i]*self.left_occ_mask[i],  self.left_pyramid[i]*self.left_occ_mask[i]) for i in range(4)]
            self.ssim_loss_left  = [tf.reduce_mean(s) / self.left_occ_mask_avg[i] for i, s in enumerate(self.ssim_left)]
            
            self.ssim_right = [self.SSIM(self.right_est[i]*self.right_occ_mask[i], self.right_pyramid[i]*self.right_occ_mask[i]) for i in range(4)]
            self.ssim_loss_right = [tf.reduce_mean(s) / self.right_occ_mask_avg[i] for i, s in enumerate(self.ssim_right)]
    ...
    def build_summaries(self):
        ...
        with tf.device('/cpu:0'):
            for i in range(4):
                ...
                tf.summary.image('occ_left_est_' + str(i), self.left_occ_mask[i], max_outputs=4, collections=self.model_collection)
                tf.summary.image('occ_right_est_' + str(i), self.right_occ_mask[i], max_outputs=4, collections=self.model_collection)
...
def disp_godard(left_img, right_img, left_feature, right_feature, opt, is_training=True):
  ...
  if is_training:
    model = MonodepthModel(params, "train", left_img, right_img, left_feature, right_feature)
    return [model.disp1, model.disp2, model.disp3, model.disp4], model.total_loss
  else:
    model = MonodepthModel(params, "test", left_img, right_img, left_feature, right_feature)
