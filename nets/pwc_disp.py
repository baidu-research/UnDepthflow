...
def optical_flow_decoder_dc(inputs, level):
  with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
      ...
      flow_x = slim.conv2d(tf.concat([cnv4, cnv5], axis=3), 1, [3, 3], stride=1, scope="cnv6_fd_"+str(level), activation_fn=None)
...
def context_net(inputs):
    with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
      ...
      flow_x = slim.conv2d(cnv6, 1, [3, 3], rate=1, scope="cnv7_cn", activation_fn=None)
      flow_y = tf.zeros_like(flow_x)
      flow = tf.concat([flow_x, flow_y], axis=3)
...
def construct_model_pwc_full_disp(feature1, feature2, image1, neg=False):
  ...
  flow4, _ = optical_flow_decoder_dc(tf.concat([cv4, feature1_4, flow5to4[:,:,:,0:1]], axis=3), level=4)
  ...
  flow3, _ = optical_flow_decoder_dc(tf.concat([cv3, feature1_3, flow4to3[:,:,:,0:1]], axis=3), level=3)
  ...
  flow2_raw, f2 = optical_flow_decoder_dc(tf.concat([cv2, feature1_2, flow3to2[:,:,:,0:1]], axis=3), level=2) 
  ...
  flow2 = context_net(tf.concat([flow2_raw[:,:,:,0:1], f2], axis=3)) + flow2_raw
  if neg:
  ...
  disp2 = tf.image.resize_bilinear(flow4[:,:,:,0:1]/(W/(2**4)), [H//4, W//4])
  disp3 = tf.image.resize_bilinear(flow5[:,:,:,0:1]/(W/(2**5)), [H//8, W//8])
...
def pwc_disp(image1, image2, feature1, feature2):
  ...
  with tf.variable_scope('left_disp'):
    ltr_disp = construct_model_pwc_full_disp(feature1, feature2, image1, neg=True)
  
  with tf.variable_scope('right_disp'):  
    rtl_disp = construct_model_pwc_full_disp(feature2, feature1, image2, neg=False)
  
  return [tf.concat([ltr+min_disp, rtl+min_disp], axis=3) for ltr, rtl in zip(ltr_disp, rtl_disp)]
