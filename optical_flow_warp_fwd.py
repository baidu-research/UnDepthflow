...
def transformerFwd(U, flo, out_size, name='SpatialTransformerFwd', 
    ...
    def _interpolate(im, x, y, out_size):
        with tf.variable_scope('_interpolate'):
            ...
            wa = tf.where(tf.logical_and(tf.equal(x0_c, x0), tf.equal(y0_c, y0)), wa, zerof)
            wb = tf.where(tf.logical_and(tf.equal(x0_c, x0), tf.equal(y1_c, y1)), wb, zerof)
            wc = tf.where(tf.logical_and(tf.equal(x1_c, x1), tf.equal(y0_c, y0)), wc, zerof)
            wd = tf.where(tf.logical_and(tf.equal(x1_c, x1), tf.equal(y1_c, y1)), wd, zerof)
