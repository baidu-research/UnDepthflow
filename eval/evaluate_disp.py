...
def eval_disp_avg(pred_disps, path, disp_num=None, moving_masks=None):
  ...
  for i, pred_disp in enumerate(pred_disps):
    if disp_num is not None:
      gt_disp = sm.imread(path + "/disp_occ_" + str(disp_num) +"/" + str(i).zfill(6) + "_10.png", -1)
      gt_disp_noc = sm.imread(path + "/disp_noc_" + str(disp_num) + "/" + str(i).zfill(6) + "_10.png", -1)
    else:
      gt_disp = sm.imread(path + "/disp_occ/" + str(i).zfill(6) + "_10.png", -1)
      gt_disp_noc = sm.imread(path + "/disp_noc/" + str(i).zfill(6) + "_10.png", -1)
    ...
    pred_disp = W * cv2.resize(pred_disp, (W, H), interpolation=cv2.INTER_LINEAR)
    ...
    error_occ += calculate_error_rate(epe_map, gt_disp, valid_mask - noc_mask)
    ...
    if moving_masks:
      ...
      error_move += calculate_error_rate(epe_map, gt_disp, valid_mask*move_mask)
      error_static += calculate_error_rate(epe_map, gt_disp, valid_mask*(1.0-move_mask))
  
  if moving_masks:
    result = "{:>10}, {:>10}, {:>10}, {:>10}, {:>10}, {:>10} \n".format('epe', 'noc_rate', 'occ_rate', 'move_rate',
                                                                        'static_rate', 'err_rate')
    result += "{:10.4f}, {:10.4f}, {:10.4f}, {:10.4f}, {:10.4f}, {:10.4f} \n".format(error / num, error_noc/num, error_occ / num, 
                                                                                     error_move / num, error_static / num, error_rate / num)
    return result
  else:
    result = "{:>10}, {:>10}, {:>10}, {:>10} \n".format('epe', 'noc_rate', 'occ_rate', 'err_rate')
