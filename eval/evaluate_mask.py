...
def read_mask_gt_worker(i):
  return sm.imread(FLAGS.gt_2015_dir + "/obj_map/" + str(i).zfill(6) + "_10.png", -1)
...
def eval_mask(pred_masks, gt_masks, opt):
  ...
  for i in range(200):
    ...
    pred_mask = cv2.resize(pred_masks[i], (W, H), interpolation=cv2.INTER_LINEAR)
    
    pred_mask[pred_mask >= 0.5] = 1.0
    pred_mask[pred_mask < 0.5] = 0.0

    sm.imsave(os.path.join(opt.trace, "pred_mask", str(i).zfill(6)+"_10_plot.png"), grey_cmap(pred_mask))
    sm.imsave(os.path.join(opt.trace, "pred_mask", str(i).zfill(6)+"_10.png"), pred_mask)
