...
def load_gt_disp_kitti(path, eval_occ):
    ...
    for i in range(200):
        if eval_occ:
          disp = sm.imread(path + "/disp_occ_0/" + str(i).zfill(6) + "_10.png", -1)
        else:
          disp = sm.imread(path + "/disp_noc_0/" + str(i).zfill(6) + "_10.png", -1)
...
def write_test_results(test_result_flow_optical, test_result_disp, test_result_disp2, test_image1, opt, mode):
  ...
  for flow, disp0, disp1, img1, i in zip(test_result_flow_optical, test_result_disp, test_result_disp2, test_image1, range(len(test_image1))):
    H, W = img1.shape[0:2]
    flow[:,:,0] = flow[:,:,0] / opt.img_width * W
    flow[:,:,1] = flow[:,:,1] / opt.img_height * H  
     
    flow = cv2.resize(flow, (W, H), interpolation=cv2.INTER_LINEAR)
    write_flow_png(flow, os.path.join(output_dir, mode, "flow", str(i).zfill(6)+"_10.png"))
    
    disp0 = W * cv2.resize(disp0, (W, H), interpolation=cv2.INTER_LINEAR)
    skimage.io.imsave(os.path.join(output_dir, mode, "disp_0", str(i).zfill(6)+"_10.png"), 
                      (disp0*256).astype('uint16'))

    disp1 = W * cv2.resize(disp1, (W, H), interpolation=cv2.INTER_LINEAR)
    skimage.io.imsave(os.path.join(output_dir, mode, "disp_1", str(i).zfill(6)+"_10.png"), 
