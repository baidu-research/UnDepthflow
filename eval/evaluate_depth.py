...
def load_depths(pred_disp_org, gt_path, eval_occ):
    ...
    gt_depths, pred_depths, pred_disparities_resized = convert_disps_to_depths_kitti(gt_disparities, pred_disp_org)
...
def eval_depth(gt_depths, pred_depths, gt_disparities, pred_disparities_resized, min_depth=1e-3, max_depth=80):
    ...
    for i in range(num_samples):
        ...
        gt_depth, pred_depth, mask = process_depth(gt_depth, pred_depth, gt_disparities[i], min_depth, max_depth)
    ...
    return abs_rel.mean(), sq_rel.mean(), rms.mean(), log_rms.mean(), a1.mean(), a2.mean(), a3.mean(), d1_all.mean()
