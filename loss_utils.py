...
def mean_charb_error(true, pred, beta):
    return tf.reduce_sum(
        tf.sqrt((tf.square(beta * (true - pred)) + 0.001 * 0.001
...
def cal_grad_error(flo, image, beta):
  ...
  """
    ...
    error += mean_charb_error_wmask(flo[:, 1:, :, :], flo[:, :-1, :, :],
                                    weights_y, beta)
    error += mean_charb_error_wmask(flo[:, :, 1:, :], flo[:, :, :-1, :],
                                    weights_x, beta)
...
def get_image_grad(image, scale=1.0):
    ...
    return tf.concat(
        [
            image, (image - image_pad[:, 1:-1, 0:-2, :]) * scale,
            ...
            (image - image_pad[:, 2:, 2:, :]) * scale
        ],
        axis=3)


def get_image_grad2(image):
    batch_size, img_height, img_width, color_channels = map(
        int, image.get_shape()[0:4])
    ...
    for i in range(5):
        for j in range(5):
            if i != 2 or j != 2:
                grads.append(image - image_pad[:, i:(i + img_height), j:(
                    j + img_width), :])
...
def get_channel(image):
    ...
    for i in range(10):
        channels.append(
            tf.where(
                tf.logical_and(image >= i / 10.0, image < (i + 1) / 10.0),
                ones, zeros))
...
def get_reference_explain_mask(downscaling, opt):
    tmp = np.array([0, 1])
    ref_exp_mask = np.tile(tmp, (opt.batch_size // opt.num_gpus,
                                 int(opt.img_height / (2**downscaling)),
                                 int(opt.img_width / (2**downscaling)), 1))
...
def compute_edge_aware_smooth_loss_1st(disp, image):
    ...
    weights_x = tf.exp(-10.0 * tf.reduce_mean(
        tf.abs(image_gradients_x), 3, keep_dims=True))
    weights_y = tf.exp(-10.0 * tf.reduce_mean(
        tf.abs(image_gradients_y), 3, keep_dims=True))
...
def compute_exp_reg_loss(pred, ref):
    l = tf.nn.softmax_cross_entropy_with_logits(
        labels=tf.reshape(ref, [-1, 2]), logits=tf.reshape(pred, [-1, 2]))
