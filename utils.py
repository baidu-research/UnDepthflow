...
def inverse_warp(depth, pose, intrinsics, intrinsics_inv, pose_mat_inverse=False):
    ...
    def _meshgrid_abs_xy(batch, height, width):
        ...
        x_t = (x_t + 1.0) * 0.5 * tf.cast(width, tf.float32)
        y_t = (y_t + 1.0) * 0.5 * tf.cast(height, tf.float32)
        return tf.tile(tf.expand_dims(x_t, 0), [batch, 1, 1]), tf.tile(tf.expand_dims(y_t, 0), [batch, 1, 1])
    ...
    tgt_pixel_coords_x, tgt_pixel_coords_y = _meshgrid_abs_xy(batch_size, img_height, img_width)
    flow_x = src_pixel_coords[:, :, :, 0] - tgt_pixel_coords_x
    flow_y = src_pixel_coords[:, :, :, 1] - tgt_pixel_coords_y
    flow = tf.concat([tf.expand_dims(flow_x, -1), tf.expand_dims(flow_y, -1)], axis=-1)
...
def calculate_pose_basis(cam_coords1, cam_coords2, weights, batch_size):
    ...
    '''
    centroids1 = tf.reduce_mean(cam_coords1*weights, axis=2, keep_dims=True) / tf.reduce_mean(weights, axis=2, keep_dims=True)
    centroids2 = tf.reduce_mean(cam_coords2*weights, axis=2, keep_dims=True) / tf.reduce_mean(weights, axis=2, keep_dims=True)
    
    cam_coords1_shifted = tf.expand_dims(tf.transpose(cam_coords1 - centroids1, [0, 2, 1]), -1)
    cam_coords2_shifted = tf.expand_dims(tf.transpose(cam_coords2 - centroids2, [0, 2, 1]), -2)
    
    weights_trans = tf.expand_dims(tf.transpose(weights, [0, 2, 1]), -1)
    H = tf.reduce_sum(tf.matmul(cam_coords1_shifted, cam_coords2_shifted)*weights_trans, axis=1, keep_dims=False)
    S, U, V = tf.svd(H)
...
def inverse_warp_new(depth1, depth2, pose, intrinsics, intrinsics_inv, flow_input, occu_mask, pose_mat_inverse=False):
    """
    ...
    cam_coords2 = tf.reshape(cam_coords2, [batch_size, 3, img_height, img_width])
    cam_coords2 = tf.transpose(cam_coords2, [0, 2, 3, 1])
    cam_coords2_trans = transformer_old(cam_coords2, flow_input, [img_height, img_width])
    # Point Cloud \tilda{Q_1}
    cam_coords2_trans = tf.reshape(tf.transpose(cam_coords2_trans, [0, 3, 1, 2]), [batch_size, 3, -1])
    
    occu_mask = tf.reshape(occu_mask, [batch_size, 1, -1])
    # To eliminate occluded area from the small_mask
    occu_mask = tf.where(occu_mask < 0.75, tf.ones_like(occu_mask)*10000.0, tf.ones_like(occu_mask))
    
    diff2 = tf.sqrt(tf.reduce_sum(tf.square(cam_coords1_trans - cam_coords2_trans), axis=1, keep_dims=True)) * occu_mask
    small_mask = tf.where(diff2 < tf.contrib.distributions.percentile(diff2, 25.0, axis=2, keep_dims=True), 
                          tf.ones_like(diff2), tf.zeros_like(diff2))
    
    # Delta T
    rigid_pose_mat = calculate_pose_basis(cam_coords1_trans, cam_coords2_trans, small_mask, batch_size)    
    # T' = deltaT x T
    ...
    cam_coords1_trans_z = tf.reshape(cam_coords1_trans_z, [batch_size, img_height, img_width, 1])
    disp1_trans = 1.0 / cam_coords1_trans_z
    
    return flow, pose_mat2, disp1_trans, tf.reshape(small_mask, [batch_size, img_height, img_width, 1])
