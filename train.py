...
flags.DEFINE_string('data_dir', '',
                    'root filepath of data.')
flags.DEFINE_string('train_file', './filenames/kitti_train_files_png_4frames.txt',
                    'training file')
...
def main(unused_argv):
  ...
  with tf.Graph().as_default(), tf.device('/cpu:0'):
    ...
    image1, image_r, image2, image2_r, proj_cam2pix, proj_pix2cam = MonodepthDataloader(FLAGS).data_batch
    ...
    split_cam2pix = tf.split(axis=0, num_or_size_splits=FLAGS.num_gpus, value=proj_cam2pix)
    split_pix2cam = tf.split(axis=0, num_or_size_splits=FLAGS.num_gpus, value=proj_pix2cam)
    split_image_r = tf.split(axis=0, num_or_size_splits=FLAGS.num_gpus, value=image_r)
    split_image_r_next = tf.split(axis=0, num_or_size_splits=FLAGS.num_gpus, value=image2_r)
          ...
          with tf.name_scope(scopename) as ns:
            if i == 0:
              model = Model(split_image1[i], split_image2[i], split_image_r[i], split_image_r_next[i], 
                            split_cam2pix[i], split_pix2cam[i], 
    ...
    for itr in range(start_itr, FLAGS.num_iterations):
      if FLAGS.train_test == "train":
        _, summary_str, summary_scalar_str= sess.run([apply_gradient_op, summary_op, model.summ_op])
        ...
        if (itr) % (SAVE_INTERVAL) == 2:
            saver.save(sess, FLAGS.trace + '/model', global_step=global_step)
