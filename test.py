  ...
  with tf.name_scope("evaluation"):
    ...
    if opt.eval_pose != "":
      ...
      for seq_no in seqs:
        ...
        eval_snippet(os.path.join(opt.trace, "pred_poses", seq_no),
    ...
    for eval_data in ["kitti_2012", "kitti_2015"]:
      ...
      for i in range(total_img_num):
          img1 = sm.imread(os.path.join(gt_dir, "image_2", str(i).zfill(6)+"_10.png"))
          img1_orig = img1
          orig_H, orig_W = img1.shape[0:2]
          img1 = sm.imresize(img1, (opt.img_height, opt.img_width))
          
          img2 = sm.imread(os.path.join(gt_dir, "image_2", str(i).zfill(6)+"_11.png"))
          img2 = sm.imresize(img2, (opt.img_height, opt.img_width))
          
          imgr = sm.imread(os.path.join(gt_dir, "image_3", str(i).zfill(6)+"_10.png"))
          imgr = sm.imresize(imgr, (opt.img_height, opt.img_width))

          img2r = sm.imread(os.path.join(gt_dir, "image_3", str(i).zfill(6)+"_11.png"))
          ...
          calib_file = os.path.join(gt_dir, "calib_cam_to_cam", str(i).zfill(6)+".txt")
      ...
      if opt.eval_depth and eval_data == "kitti_2015":
        ...
        gt_depths, pred_depths, gt_disparities, pred_disp_resized = load_depths(test_result_disp, gt_dir, eval_occ=True)
        abs_rel, sq_rel, rms, log_rms, a1, a2, a3, d1_all = eval_depth(gt_depths, pred_depths, gt_disparities, pred_disp_resized)
        sys.stderr.write("{:10.4f}, {:10.4f}, {:10.3f}, {:10.3f}, {:10.3f}, {:10.3f}, {:10.3f}, {:10.3f} \n".format(abs_rel, sq_rel, rms, log_rms, d1_all, a1, a2, a3))
